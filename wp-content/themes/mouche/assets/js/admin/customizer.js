...
( function( wp, $ ) {
	...
	api.SFGuidedTour = {
		...
		_hideTour: function( remove ) {
			...
			$( 'body' ).addClass( 'sf-exiting' ).on( 'animationend.mouche webkitAnimationEnd.mouche', function() {
				$( this ).removeClass( 'sf-exiting' ).off( 'animationend.mouche webkitAnimationEnd.mouche' ).addClass( 'sf-hidden' );
				self.$container.hide();
		...
		_revealTour: function() {
			...
			$( 'body' ).addClass( 'sf-entering' ).on( 'animationend.mouche webkitAnimationEnd.mouche', function() {
				$( this ).removeClass( 'sf-entering' ).off( 'animationend.mouche webkitAnimationEnd.mouche' );
		...
		_moveContainer: function( $selector ) {
			...
			this.$container.addClass( 'sf-moving' ).css({ 'transform': 'translateY(' + parseInt( position, 10 ) + 'px)' }).on( 'transitionend.mouche', function() {
				self.$container.removeClass( 'sf-moving' );
				self.$container.off( 'transitionend.mouche' );
			} );
		},
